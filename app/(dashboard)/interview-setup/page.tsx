...
export default function InterviewSetupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast({
        title: 'Success!',
        description: `Interview started with ${result.total_planned_questions} questions`,
      });
    ...
    <div className="container mx-auto py-8 px-4 max-w-4xl">
      <Card className="border-2">
        <CardHeader>
          ...
          <CardDescription>
            Configure your mock interview with AI-powered question generation
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <Label>Voice-Based Interview? *</Label>
              <RadioGroup
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <Label htmlFor="voice-yes" className="font-normal cursor-pointer">
                    Yes (AI will speak questions & listen to answers)
                  </Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="no" id="voice-no" />
                  <Label htmlFor="voice-no" className="font-normal cursor-pointer">No (Text only)</Label>
                </div>
              </RadioGroup>
            </div>

            <div className="space-y-2">
              <Label htmlFor="mockInterviewGoal">Interview Goal *</Label>
              <Input
                id="mockInterviewGoal"
                value={formData.mockInterviewGoal}
                onChange={(e) => setFormData({ ...formData, mockInterviewGoal: e.target.value })}
                placeholder="e.g., Internship Prep, Job Interview Practice"
                required
              />
            </div>
            ...
            <div className="pt-4">
              <Button type="submit" className="w-full" size="lg" disabled={isSubmitting}>
                {isSubmitting ? 'Starting Interview...' : 'Start AI Interview'}
              </Button>
            </div>
          </form>
        </CardContent>
